...
from .formals import (CUSTOMER_ID_ANONYMOUS, KTI1, BankIdentifier,
                      DescriptionRequired, SupportedMessageTypes,
...
from .security import (PinTanDummyEncryptionMechanism,
...
from .segments.debit import (HKDBS1, HKDBS2, HKDMB1, HKDMC1, HKDME1, HKDME2,
...
from .segments.statement import DKKKU2, HKCAZ1, HKKAZ5, HKKAZ6, HKKAZ7
...
class TransactionResponse:
        ...
        for hirms in response_message.find_segments(HIRMS2):
            for resp in hirms.responses:
                self.set_status_if_higher(
                    _RESPONSE_STATUS_MAPPING.get(
                        resp.code[0], ResponseStatus.UNKNOWN
                    )
                )
    ...
    def __repr__(self):
        return "<{o.__class__.__name__}(status={o.status!r}, responses={o.responses!r}, data={o.data!r})>".format(
            o=self
        )
...
class FinTS3Client:
    def __init__(
        ...
        if isinstance(bank_identifier, BankIdentifier):
            ...
            self.bank_identifier = BankIdentifier(
                BankIdentifier.COUNTRY_ALPHA_TO_NUMERIC['DE'], bank_identifier
            )
        else:
            raise TypeError(
                "bank_identifier must be BankIdentifier or str (BLZ)"
            )
    ...
    def process_response_message(
        self, dialog, message: FinTSInstituteMessage, internal_send=True
    ):
        ...
        if upa:
            ...
            self.upd = SegmentSequence(message.find_segments('HIUPD'))
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if self._standing_dialog:
            if exc_type is not None and issubclass(
                exc_type, FinTSSCARequiredError
            ):
    ...
    def _get_dialog(self, lazy_init=False):
        if lazy_init and self._standing_dialog:
            raise Exception(
                "Cannot _get_dialog(lazy_init=True) with _standing_dialog"
            )
    ...
    def _deconstruct_v1(self, including_private=False):
        data = {
            ...
            "bpa_bin": FinTS3Serializer().serialize_message(self.bpa)
            if self.bpa
            else None,
            ...
            data.update(
                {
                    ...
                    "upa_bin": FinTS3Serializer().serialize_message(self.upa)
                    if self.upa
                    else None,
    ...
    def get_information(self):
        ...
        if self.bpd.segments:
            retval['bank']['supported_operations'] = {
                op: any(
                    self.bpd.find_segment_first(cmd[0] + 'I' + cmd[2:] + 'S')
                    for cmd in op.value
                )
            ...
            for op in FinTSOperations:
                for segment in (
                    ...
                    if not hasattr(segment, 'parameter'):
                        continue
                    formats = getattr(
                        segment.parameter, 'supported_sepa_formats', []
                    )
                    retval['bank']['supported_formats'][op] = list(
                        set(
                            retval['bank']['supported_formats'].get(op, [])
                        ).union(set(formats))
                    )
            hispas = self.bpd.find_segment_first('HISPAS')
            if hispas:
                retval['bank']['supported_sepa_formats'] = list(
                    hispas.parameter.supported_sepa_formats
                )
            else:
        ...
        if self.upd.segments:
            for upd in self.upd.find_segments('HIUPD'):
                ...
                acc[
                    'subaccount_number'
                ] = upd.account_information.subaccount_number
                acc[
                    'bank_identifier'
                ] = upd.account_information.bank_identifier
                ...
                acc['supported_operations'] = {
                    op: any(
                        allowed_transaction.transaction in op.value
    ...
    def _get_sepa_accounts(self, command_seg, response):
        ...
        return [
            a for a in [acc.as_sepa_account() for acc in self.accounts] if a
        ]
    ...
    def get_sepa_accounts(self):
        ...
        with self._get_dialog() as dialog:
            return self._send_with_possible_retry(
                dialog, seg, self._get_sepa_accounts
            )

    def _continue_fetch_with_touchdowns(self, command_seg, response):
        for resp in response.response_segments(
            command_seg, *self._touchdown_args, **self._touchdown_kwargs
        ):
        ...
        if touchdown:
            logger.info(
                'Fetching more results ({})...'.format(self._touchdown_counter)
            )
        ...
        if touchdown:
            ...
            return self._send_with_possible_retry(
                self._touchdown_dialog,
                seg,
                self._continue_fetch_with_touchdowns,
            )
        else:
            return self._touchdown_response_processor(
                self._touchdown_responses
            )

    def _fetch_with_touchdowns(
        self, dialog, segment_factory, response_processor, *args, **kwargs
    ):
        ...
        return self._send_with_possible_retry(
            dialog, seg, self._continue_fetch_with_touchdowns
        )
    ...
    def _find_highest_supported_command(self, *segment_classes, **kwargs):
        ...
        return_parameter_segment = kwargs.get(
            "return_parameter_segment", False
        )

        parameter_segment_name = "{}I{}S".format(
            segment_classes[0].TYPE[0], segment_classes[0].TYPE[2:]
        )
        version_map = dict((clazz.VERSION, clazz) for clazz in segment_classes)
        max_version = self.bpd.find_segment_highest_version(
            parameter_segment_name, version_map.keys()
        )
        if not max_version:
            raise FinTSUnsupportedOperation(
                'No supported {} version found. I support {}, bank supports {}.'.format(
                    parameter_segment_name,
                    tuple(version_map.keys()),
                    tuple(
                        v.header.version
                        for v in self.bpd.find_segments(parameter_segment_name)
                    ),
                )
            )
        ...
        with self._get_dialog() as dialog:
            ...
            responses = self._fetch_with_touchdowns(
                ...
                lambda touchdown: dkkku(
                    account=dkkku._fields['account'].type.from_sepa_account(
                        account
                    )
                    if account
                    else None,
    ...
    def get_balance(self, account: SEPAAccount):
        ...
        with self._get_dialog() as dialog:
            hksal = self._find_highest_supported_command(
            ...
            response = self._send_with_possible_retry(
                dialog, seg, self._get_balance
            )
            return response
    ...
    def get_status_protocol(self):
        with self._get_dialog() as dialog:
            ...
            responses = self._fetch_with_touchdowns(
                dialog,
                lambda touchdown: hkpro(touchdown_point=touchdown,),
                lambda responses: responses,
                'HIPRO',
            )
    ...
    def get_communication_endpoints(self):
        with self._get_dialog() as dialog:
            ...
            responses = self._fetch_with_touchdowns(
                dialog,
                lambda touchdown: hkkom(touchdown_point=touchdown,),
                lambda responses: responses,
                'HIKOM',
            )
    ...
    def _find_supported_sepa_version(self, candidate_versions):
        ...
        if not hispas:
            logger.warning(
                "Could not determine supported SEPA versions, is the dialogue open? Defaulting to first candidate: %s.",
        ...
        for candidate in candidate_versions:
            if (
                "urn:iso:std:iso:20022:tech:xsd:{}".format(candidate)
                in bank_supported
            ):
                return candidate
            if (
                "urn:iso:std:iso:20022:tech:xsd:{}.xsd".format(candidate)
                in bank_supported
            ):
                return candidate

        logger.warning(
            "No common supported SEPA version. Defaulting to first candidate and hoping for the best: %s.",
            candidate_versions[0],
        )
        return candidate_versions[0]
    ...
    def simple_sepa_transfer(
        ...
        version = self._find_supported_sepa_version(
            ['pain.001.001.03', 'pain.001.003.03']
        )
        ...
        return self.sepa_transfer(
            account,
            xml,
            pain_descriptor="urn:iso:std:iso:20022:tech:xsd:" + version,
        )
    ...
    def sepa_transfer(
        ...
        with self._get_dialog() as dialog:
            ...
            hiccxs, hkccx = self._find_highest_supported_command(
                command_class, return_parameter_segment=True
            )

            seg = hkccx(
                account=hkccx._fields['account'].type.from_sepa_account(
                    account
                ),
            ...
            if multiple:
                if (
                    hiccxs.parameter.sum_amount_required
                    and control_sum is None
                ):
                    raise ValueError("Control sum required.")
                if (
                    book_as_single
                    and not hiccxs.parameter.single_booking_allowed
                ):
                    raise FinTSUnsupportedOperation(
                        "Single booking not allowed by bank."
                    )
            ...
            return self._send_with_possible_retry(
                dialog, seg, self._continue_sepa_transfer
            )
    ...
    def sepa_debit(
        ...
        with self._get_dialog() as dialog:
            ...
            hidxxs, hkdxx = self._find_highest_supported_command(
                *command_candidates, return_parameter_segment=True
            )

            seg = hkdxx(
                account=hkdxx._fields['account'].type.from_sepa_account(
                    account
                ),
                sepa_descriptor=pain_descriptor,
                sepa_pain_message=pain_message.encode(),
            ...
            if multiple:
                if (
                    hidxxs.parameter.sum_amount_required
                    and control_sum is None
                ):
                    raise ValueError("Control sum required.")
                if (
                    book_as_single
                    and not hidxxs.parameter.single_booking_allowed
                ):
                    raise FinTSUnsupportedOperation(
                        "Single booking not allowed by bank."
                    )
            ...
            return self._send_with_possible_retry(
                dialog, seg, self._continue_sepa_debit
            )
...
class NeedTANResponse(NeedRetryResponse):
    ...
    def __repr__(self):
        return '<o.__class__.__name__(command_seg={o.command_seg!r}, tan_request={o.tan_request!r})>'.format(
            o=self
        )
        ...
        if hasattr(self.tan_request, 'challenge_hhduc'):
            if self.tan_request.challenge_hhduc:
                if len(self.tan_request.challenge_hhduc) < 256:
                    self.challenge_hhduc = self.tan_request.challenge_hhduc.decode(
                        'us-ascii'
                    )
                else:
                    ...
                    if len(type_len_field) == 2:
                        ...
                        if len(content_len_field) == 2:
                            ...
                            self.challenge_matrix = (
                                type_data.decode('us-ascii', 'replace'),
                                content_data,
                            )
        ...
        if self.challenge.startswith('CHLGUC  '):
            ...
            if l.isdigit():
                ...
                if self.challenge_hhduc.startswith('iVBO'):
                    self.challenge_matrix = (
                        'image/png',
                        b64decode(self.challenge_hhduc),
                    )
                    self.challenge_hhduc = None
...
class FinTS3PinTanClient(FinTS3Client):
    ...
    def _new_dialog(self, lazy_init=False):
        if self.pin is None:
            ...
            not self.selected_security_function
            or self.selected_security_function == '999'
        ):
            ...
            auth = [
                PinTanTwoStepAuthenticationMechanism(
                    self, self.selected_security_function, self.pin,
                )
            ]

        return FinTSDialog(
            self, lazy_init=lazy_init, enc_mechanism=enc, auth_mechanisms=auth,
        )
    ...
    def _ensure_system_id(self):
        if (
            self.system_id != SYSTEM_ID_UNASSIGNED
            or self.user_id == CUSTOMER_ID_ANONYMOUS
        ):
            return

        with self._get_dialog(lazy_init=True) as dialog:
            response = dialog.init(HKSYN3(SynchronizationMode.NEW_SYSTEM_ID),)
    ...
    def is_tan_media_required(self):
        tan_mechanism = self.get_tan_mechanisms()[
            self.get_current_tan_mechanism()
        ]
        return (
            getattr(tan_mechanism, 'supported_media_number', None) is not None
            and tan_mechanism.supported_media_number > 1
    ...
    def _process_response(self, dialog, segment, response):
        if response.code == '3920':
            ...
            if (
                self.selected_security_function is None
                or not self.selected_security_function
                in self.allowed_security_functions
            ):
                # Select the first available twostep security_function that we support
                for (
                    security_function,
                    parameter,
                ) in self.get_tan_mechanisms().items():
        ...
        if (
            (not dialog.open and response.code.startswith('9'))
            ...
            if self.pin:
                self.pin.block()
            raise FinTSClientPINError(
                "Error during dialog initialization, PIN wrong?"
            )
        ...
        if response.code == '3938':
            ...
            if self.pin:
                self.pin.block()
            raise FinTSClientTemporaryAuthError(
    ...
    def set_tan_mechanism(self, security_function):
        if self._standing_dialog:
            raise Exception(
                "Cannot change TAN mechanism with a standing dialog"
            )
    ...
    def get_tan_media(
        self, media_type=TANMediaType2.ALL, media_class=TANMediaClass4.ALL
    ):
        ...
        with context as dialog:
            ...
            seg = hktab(
                tan_media_type=media_type, tan_media_class=str(media_class),
            )
